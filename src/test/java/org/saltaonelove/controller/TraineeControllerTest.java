...
class TraineeControllerTest {
    ...
    @BeforeEach
    void setUp() {
        authRequest = new AuthRequest("trainee1", "securepass");
    }

    @Test
    void testRegisterTrainee() throws Exception {
        TraineeRegisterRequest request = new TraineeRegisterRequest("firstName", "lastName",
                LocalDate.ofYearDay(2001,1), "someAddress");

        Trainee createdTrainee = new Trainee();
        createdTrainee.setUsername("firstName.lastName");
        createdTrainee.setPassword("securepass");

        when(traineeService.registerTrainee(request)).thenReturn(createdTrainee);
